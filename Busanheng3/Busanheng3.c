...
void train_box(int, int, int, int, int, int, int, int, int, int, int, int);
void train_box2(int, int, int, int);
void train_box3(int, int, int, int, int, int);
void train_box4(int, int, int, int, int, int, int, int);
void train_box5(int, int, int, int, int, int, int, int);
void repeat(int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int);
...
int z_move(int, int, int, int, int, int, int, int, int, int, int, int, int, int, int*, int*, int*, int*, int*, int*, int*);
int z_movewhere(int, int, int, int, int, int, int, int, int, int, int, int*);
int z_movewhere2(int, int, int, int, int, int, int, int, int, int, int, int*, int*, int*, int*);
void moveresult(int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int);
void c_moveresult(int, int, int, int, int, int, int, int, int, int, int);
void v_moveresult(int, int, int, int, int);
void z_moveresult(int, int, int, int, int, int, int);
void sz_moveresult(int, int, int, int, int, int);
...
int c_action(int, int, int, int, int, int, int*);
...
int main(void) {
	...
	for (int i = 0; i < 4; i++) {
		...
		int bstm = 0, count = 0, zombie = 0, c_alive = 3, v_alive = 1, c_exit = 0;
		train_box(count, train_length, stage, c_alive, v_alive, c0_pos, c1_pos, c2_pos, v_pos, z_pos, m_pos, c_exit);
		repeat(train_length, stm, p, count, zombie, stage, c_alive, v_alive, c0_pos, c1_pos, c2_pos, v_pos, z_pos, m_pos, c_aggro, v_aggro, m_aggro, c_exit);
		++stage;
...
void train_box(int count, int train_length, int stage, int c_alive, int v_alive, int c0_pos, int c1_pos, int c2_pos, int v_pos, int z_pos, int m_pos, int c_exit) { // 기차 상자
	...
	else if (stage == 3) { train_box4(train_length, c_alive, c0_pos, c1_pos, c2_pos, z_pos, m_pos, c_exit); }
	else if (stage == 4) { train_box5(train_length, c_alive, c0_pos, c1_pos, c2_pos, z_pos, m_pos, c_exit); }
}
...
void train_box4(int train_length, int c_alive, int c0_pos, int c1_pos, int c2_pos, int z_pos, int m_pos, int c_exit) { // 기차 상자
	for (int i = 1; i <= 3; i++) {
		for (int i2 = 1; i2 <= train_length; i2++) {
			if ((i == 1) || (i == 3) || ((i == 2) && ((i2 == train_length)) || (i2 == 1))) { printf("#"); }
			else if ((i == 2) && (i2 == c0_pos) && ((c_alive == 1) || (c_alive == 2) || (c_alive == 3)) && (c_exit == 0)) { printf("C"); }
			else if ((i == 2) && (i2 == c1_pos) && ((c_alive == 2) || (c_alive == 3)) && (c_exit <= 1)) { printf("C"); }
			else if ((i == 2) && (i2 == c2_pos) && (c_alive == 3) && (c_exit <= 2)) { printf("C"); }
			else if ((i == 2) && (i2 == z_pos)) { printf("Z"); }
			else if ((i == 2) && (i2 == m_pos)) { printf("M"); }
			else if (i == 2) { printf(" "); }
		}
...
void train_box5(int train_length, int c_alive, int c0_pos, int c1_pos, int c2_pos, int z_pos, int m_pos, int c_exit) { // 기차 상자
	for (int i = 1; i <= 3; i++) {
		for (int i2 = 1; i2 <= train_length; i2++) {
			if ((i == 1) || (i == 3) || ((i == 2) & ((i2 == train_length)) || (i2 == 1))) { printf("#"); }
			else if ((i == 2) && (i2 == c0_pos) && (c_alive >= 1) && (c_exit == 0)) { printf("C"); }
			else if ((i == 2) && (i2 == c1_pos) && (c_alive == 1)) { printf("Z"); }
			else if ((i == 2) && (i2 == c1_pos) && (c_alive >= 2) && (c_exit <= 1)) { printf("C"); }
...
void repeat(int train_length, int stm, int p, int count, int zombie, int stage, int c_alive, int v_alive, int c0_pos, int c1_pos, int c2_pos, int v_pos, int z_pos, int m_pos, int c_aggro, int v_aggro, int m_aggro, int c_exit) {
	int bstm, b_stage, c_baggro, v_baggro, m_baggro, c0_bpos, c1_bpos, c2_bpos, v_bpos, z_bpos, z2_bpos, m_bpos, c_result, z_result, sz_result;
	int _c_alive, _v_alive, m_move, m_action_f, z_action_r, ATK, _zombie, _c0_pos, _c1_pos, _c2_pos, _v_pos, _z_pos, _m_pos, _stm, _v_aggro, _m_aggro, _m_move, _c_exit;
	while (1) {
		++count, ++zombie;
		...
		z_result = z_move(count, zombie, stage, c_alive, v_alive, m_aggro, c_aggro, v_aggro, c0_pos, c1_pos, c2_pos, v_pos, z_pos, m_pos, &_zombie, &_z_pos, &_c0_pos, &_c1_pos, &_c2_pos, &sz_result, &z2_bpos);
		zombie = _zombie, z_pos = _z_pos, c0_pos = _c0_pos, c1_pos = _c1_pos, c2_pos = _c2_pos, m_bpos = m_pos, m_baggro = m_aggro;
		train_box(count, train_length, stage, c_alive, v_alive, c0_pos, c1_pos, c2_pos, v_pos, z_pos, m_pos, c_exit);
		moveresult(stage, c_alive, v_alive, c_result, c0_bpos, c0_pos, c1_bpos, c1_pos, c2_bpos, c2_pos, v_bpos, v_pos, c_baggro, c_aggro, v_baggro, v_aggro, z_result, sz_result, z_bpos, z2_bpos, z_pos);
		...
		stage = c_action(c0_pos, c1_pos, c2_pos, stage, c_alive, c_exit, &_c_exit);
		c_exit = _c_exit;
		if (b_stage != stage) { break; }
		v_action(stage, c2_pos, v_pos, &_c2_pos, &_v_pos);
	...
	*_c2_pos_ = c2_pos; // 위치
	if (c_per <= p) {
		*_c_aggro = --c_aggro; // 어그로
		if (2 < stage) {
			if (c_alive == 3) { *_c2_pos_ = c2_pos; return 4; }
			else if (c_alive == 2) { *_c1_pos_ = c1_pos; return 3; }
			else if (c_alive == 1) { *_c0_pos_ = c0_pos; return 2; }
		}
		else { *_c2_pos_ = c2_pos; return 0; }
	}
	else if (c_per > p) {
		*_c_aggro = ++c_aggro;
		if (2 < stage) {
			if (c_alive == 3) {
				if ((c2_pos == (c1_pos + 1)) && (c1_pos == (c0_pos + 1))) { *_c0_pos_ = c0_pos - 1; return 5; }
				else if ((c2_pos == (c1_pos + 1))) { *_c1_pos_ = c1_pos - 1; return 6; }
				else { *_c2_pos_ = c2_pos - 1; return 7; }
			}
			else if (c_alive == 2) {
				if ((c1_pos == (c0_pos + 1))) { *_c0_pos_ = c0_pos - 1; return 5; }
				else { *_c1_pos_ = c1_pos - 1; return 6; }
			}
			else if (c_alive == 1) { *_c0_pos_ = c0_pos - 1; return 5; }
		}
		else { *_c2_pos_ = c2_pos - 1; return 1; }
	}
}
...
int z_move(int count, int zombie, int stage, int c_alive, int v_alive, int m_aggro, int c_aggro, int v_aggro, int c0_pos, int c1_pos, int c2_pos, int v_pos, int z_pos, int m_pos, int* _zombie, int* _z_pos, int* _c0_pos, int* _c1_pos, int* _c2_pos, int* sz_result, int* z2_bpos) {
	int _c0_pos_, _c1_pos_, _c2_pos_, _z_pos_, _z2_bpos, result = 0;
	*_zombie = zombie;
	*_c0_pos = c0_pos;
	*_c1_pos = c1_pos;
	*_c2_pos = c2_pos;
	*_z_pos = z_pos;
	if (count % 2 != 0) {
		if (count < zombie) {
			*_zombie = --zombie, *_z_pos = z_pos;
			result = 2;
		}
		else {
			result = z_movewhere(stage, c_alive, m_aggro, c_aggro, v_aggro, c0_pos, c1_pos, c2_pos, v_pos, z_pos, m_pos, &_z_pos_);
			*_z_pos = _z_pos_;
			*sz_result = z_movewhere2(stage, c_alive, m_aggro, c_aggro, v_aggro, c0_pos, c1_pos, c2_pos, v_pos, z_pos, m_pos, &_c0_pos_, &_c1_pos_, &_c2_pos_, &_z2_bpos);
			*_c0_pos = _c0_pos_, *_c1_pos = _c1_pos_, *_c2_pos = _c2_pos_, * z2_bpos = _z2_bpos;
		}
	}
	else {
...
int z_movewhere(int stage, int c_alive, int m_aggro, int c_aggro, int v_aggro, int c0_pos, int c1_pos, int c2_pos, int v_pos, int z_pos, int m_pos, int* _z_pos_) { // 좀비 이동 방향
	*_z_pos_ = z_pos;
	...
	if (stage == 2) {
		if (c_pos > v_pos && m_aggro <= c_aggro) {
			if (c_pos == z_pos - 1) {
				*_z_pos_ = z_pos;
				return 3;
			}
		...
		else if (c_pos < v_pos && m_aggro <= v_aggro) {
...
int z_movewhere2(int stage, int c_alive, int m_aggro, int c_aggro, int v_aggro, int c0_pos, int c1_pos, int c2_pos, int v_pos, int z_pos, int m_pos, int* _c0_pos_, int* _c1_pos_, int* _c2_pos_, int* _z2_bpos) { // 강화 좀비 이동 방향
	*_c0_pos_ = c0_pos;
	*_c1_pos_ = c1_pos;
	*_c2_pos_ = c2_pos;
	int c_pos = 0, z2_pos = 0;
	if ((c_alive == 2)) { c_pos = c1_pos, z2_pos = c2_pos, *_z2_bpos = c2_pos; }
	else if ((c_alive == 1)) { c_pos = c0_pos, z2_pos = c1_pos, *_z2_bpos = c1_pos; }
	if ((stage == 4) && (c_alive < 3)) {
		if (m_aggro <= c_aggro) {
			if (c_pos == (z2_pos - 1)) { return 1; }
			else {
				if ((c_alive == 2)) { *_c2_pos_ = c2_pos - 1; }
				else if ((c_alive == 1)) { *_c1_pos_ = c1_pos - 1; }
				return 0;
			}
		}
		else {
			if (m_pos == (z2_pos + 1)) { return 1; }
			else if (z_pos == (z2_pos + 1)) { return 1; }
			else {
				if ((c_alive == 2)) { *_c2_pos_ = c2_pos + 1; }
				else if ((c_alive == 1)) { *_c1_pos_ = c1_pos + 1; }
				return 0;
...
void moveresult(int stage, int c_alive, int v_alive, int c_result, int c0_bpos, int c0_pos, int c1_bpos, int c1_pos, int c2_bpos, int c2_pos, int v_bpos, int v_pos, int c_baggro, int c_aggro, int v_baggro, int v_aggro, int z_result, int sz_result, int z_bpos, int z2_bpos, int z_pos) {
	...
	if (stage == 4) { sz_moveresult(stage, c_alive, sz_result, z2_bpos, c1_pos, c2_pos); }
	z_moveresult(stage, c_alive, z_result, z_bpos, z_pos, c1_pos, c2_pos); // 좀비 이동 결과
}
...
void c_moveresult(int stage, int c_alive, int c_result, int c0_bpos, int c0_pos, int c1_bpos, int c1_pos, int c2_bpos, int c2_pos, int c_baggro, int c_aggro) { // 시민 이동 결과 출력
	if (2 < stage) {
		if (c_result == 2) { printf("시민 0 : 제자리 %d ( 어그로 : %d -> %d )\n", c0_pos - 1, c_baggro, c_aggro); }
		else if (c_result == 3) { printf("시민 1 : 제자리 %d ( 어그로 : %d -> %d )\n", c1_pos - 1, c_baggro, c_aggro); }
		else if (c_result == 4) { printf("시민 2 : 제자리 %d ( 어그로 : %d -> %d )\n", c2_pos - 1, c_baggro, c_aggro); }
		else if (c_result == 5) { printf("시민 0 : %d -> %d ( 어그로 : %d -> %d )\n", c0_bpos - 1, c0_pos - 1, c_baggro, c_aggro); }
		else if (c_result == 6) { printf("시민 1 : %d -> %d ( 어그로 : %d -> %d )\n", c1_bpos - 1, c1_pos - 1, c_baggro, c_aggro); }
		else if (c_result == 7) { printf("시민 2 : %d -> %d ( 어그로 : %d -> %d )\n", c2_bpos - 1, c2_pos - 1, c_baggro, c_aggro); }
	}
	else {
...
void sz_moveresult(int stage, int c_alive, int sz_result, int z2_bpos, int c1_pos, int c2_pos) { // 좀비 이동 결과 출력
	int z2_pos = 0;
...
void z_moveresult(int stage, int c_alive, int z_result, int z_bpos, int z_pos, int c1_pos, int c2_pos) { // 좀비 이동 결과 출력
	int z2_pos = 0;
	if (z_result == 0) {
		printf("좀비 : %d -> %d\n\n", z_bpos - 1, z_pos - 1);
	}
...
int c_action(int c0_pos, int c1_pos, int c2_pos, int stage, int c_alive, int c_exit, int* _c_exit) { // 시민 행동 출력
	...
	*_c_exit = c_exit;
	...
	if ((c_pos3 != -1) && (c_exit == 0)) {
		if ((c_pos3 <= 2) && (c_pos2 >= 3)) {
			if (stage <= 4) {
